...
| T26 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T26-T28 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_drafts_api.py backend/tests/test_send_mail_api.py backend/tests/test_message_detail_api.py backend/tests/test_security_hardening.py -q && npm test --prefix frontend -- --run src/mail/ComposePanel.test.tsx src/mail/MessageReader.test.tsx src/App.test.tsx && npm run build --prefix frontend && npm run test:e2e --prefix frontend -- --grep "HTML 邮件在阅读区直接渲染且脚本不会执行\|写信保存草稿后刷新可恢复内容\|纯文本模式" && git diff --check | 通过：后端22个相关用例、前端31个相关用例、构建、3个Playwright关键验收和空白检查均通过；T26按当前产品要求使用DOMPurify净化后在正常阅读区直接展示富文本，暂不使用sandbox iframe隔离 | 2026-05-08T11:28:55+08:00 | HTML 邮件安全渲染升级 |
| T27 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T26-T28 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_drafts_api.py backend/tests/test_send_mail_api.py backend/tests/test_message_detail_api.py backend/tests/test_security_hardening.py -q && npm test --prefix frontend -- --run src/mail/ComposePanel.test.tsx src/mail/MessageReader.test.tsx src/App.test.tsx && npm run build --prefix frontend && npm run test:e2e --prefix frontend -- --grep "HTML 邮件在 sandbox iframe 中渲染且脚本不会执行\|写信保存草稿后刷新可恢复内容\|纯文本模式" && git diff --check | 通过：后端22个相关用例、前端31个相关用例、构建、3个Playwright关键验收和空白检查均通过；T27完成PATCH草稿更新、30秒自动保存和刷新恢复 | 2026-05-08T11:28:55+08:00 | 草稿自动保存协议对齐 |
| T28 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T26-T28 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_drafts_api.py backend/tests/test_send_mail_api.py backend/tests/test_message_detail_api.py backend/tests/test_security_hardening.py -q && npm test --prefix frontend -- --run src/mail/ComposePanel.test.tsx src/mail/MessageReader.test.tsx src/App.test.tsx && npm run build --prefix frontend && npm run test:e2e --prefix frontend -- --grep "HTML 邮件在 sandbox iframe 中渲染且脚本不会执行\|写信保存草稿后刷新可恢复内容\|纯文本模式" && git diff --check | 通过：后端22个相关用例、前端31个相关用例、构建、3个Playwright关键验收和空白检查均通过；T28完成富文本/纯文本切换且纯文本发送不带html_body | 2026-05-08T11:28:55+08:00 | 纯文本编辑模式 |
| T29 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T29-T31 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_attachment_upload_api.py backend/tests/test_send_mail_api.py backend/tests/test_drafts_api.py backend/tests/test_message_detail_api.py backend/tests/test_security_hardening.py -q && npm test --prefix frontend -- --run src/mail/ComposePanel.test.tsx src/mail/MessageReader.test.tsx src/App.test.tsx && npm run build --prefix frontend && npm run test:e2e --prefix frontend -- --grep "HTML 邮件在 sandbox iframe 中渲染且脚本不会执行\|写信保存草稿后刷新可恢复内容\|纯文本模式\|写信可上传多个附件并看到分块进度\|写信可插入内嵌图片并调整位置和大小后发送草稿\|写信支持上传附件、保存草稿、发送并防重复发送" && git diff --check | 通过：后端28个相关用例、前端37个相关用例、构建、6个Playwright关键验收和空白检查均通过；T29完成To/Cc/Bcc Tag输入、删除、键盘导航和重复校验 | 2026-05-08T11:52:32+08:00 | 收件人 Tag 输入框 |
| T30 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T29-T31 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_attachment_upload_api.py backend/tests/test_send_mail_api.py backend/tests/test_drafts_api.py backend/tests/test_message_detail_api.py backend/tests/test_security_hardening.py -q && npm test --prefix frontend -- --run src/mail/ComposePanel.test.tsx src/mail/MessageReader.test.tsx src/App.test.tsx && npm run build --prefix frontend && npm run test:e2e --prefix frontend -- --grep "HTML 邮件在 sandbox iframe 中渲染且脚本不会执行\|写信保存草稿后刷新可恢复内容\|纯文本模式\|写信可上传多个附件并看到分块进度\|写信可插入内嵌图片并调整位置和大小后发送草稿\|写信支持上传附件、保存草稿、发送并防重复发送" && git diff --check | 通过：后端28个相关用例、前端37个相关用例、构建、6个Playwright关键验收和空白检查均通过；T30复用FileReader/FormData/contenteditable完成内嵌图片、位置尺寸调整和HTML引用保留 | 2026-05-08T11:52:32+08:00 | 内嵌图片增强 |
| T31 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T29-T31 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_attachment_upload_api.py backend/tests/test_send_mail_api.py backend/tests/test_drafts_api.py backend/tests/test_message_detail_api.py backend/tests/test_security_hardening.py -q && npm test --prefix frontend -- --run src/mail/ComposePanel.test.tsx src/mail/MessageReader.test.tsx src/App.test.tsx && npm run build --prefix frontend && npm run test:e2e --prefix frontend -- --grep "HTML 邮件在 sandbox iframe 中渲染且脚本不会执行\|写信保存草稿后刷新可恢复内容\|纯文本模式\|写信可上传多个附件并看到分块进度\|写信可插入内嵌图片并调整位置和大小后发送草稿\|写信支持上传附件、保存草稿、发送并防重复发送" && git diff --check | 通过：后端28个相关用例、前端37个相关用例、构建、6个Playwright关键验收和空白检查均通过；T31复用FastAPI UploadFile/FormData/原生拖拽和Redis临时附件存储完成拖拽与分块上传 | 2026-05-08T11:52:32+08:00 | 附件拖拽与分块上传 |
| T32 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T32-T34 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests -q && npm test --prefix frontend && npm run build --prefix frontend && docker compose run --rm -v "/Users/mac/项目/webmail-邮件系统/backend:/app" backend alembic -c alembic.ini upgrade head && python3 scripts/plan_status.py validate && git diff --check | 通过：默认签名自动插入覆盖新建、回复、避免重复、保存草稿和发送 payload；后端97个用例、前端51个用例、前端构建、PostgreSQL Alembic upgrade head、计划校验和空白检查均通过 | 2026-05-08T12:51:24+08:00 | 默认签名自动插入 |
| T33 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T32-T34 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests -q && npm test --prefix frontend && npm run build --prefix frontend && docker compose run --rm -v "/Users/mac/项目/webmail-邮件系统/backend:/app" backend alembic -c alembic.ini upgrade head && python3 scripts/plan_status.py validate && git diff --check | 通过：签名 CRUD、默认唯一性、删除回选默认、未登录和404覆盖；后端97个用例、前端51个用例、前端构建、PostgreSQL Alembic upgrade head、计划校验和空白检查均通过 | 2026-05-08T12:51:24+08:00 | 签名管理 CRUD |
| T34 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T32-T34 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests -q && npm test --prefix frontend && npm run build --prefix frontend && docker compose run --rm -v "/Users/mac/项目/webmail-邮件系统/backend:/app" backend alembic -c alembic.ini upgrade head && python3 scripts/plan_status.py validate && git diff --check | 通过：富文本签名设置页支持图片和链接、保存前净化、列表预览净化；后端97个用例、前端51个用例、前端构建、PostgreSQL Alembic upgrade head、计划校验和空白检查均通过 | 2026-05-08T12:51:24+08:00 | 富文本签名编辑器 |
| T35 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T35-T39 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_contacts_api.py backend/tests/test_send_mail_api.py -q && npm test --prefix frontend -- --run src/App.test.tsx && npm run build --prefix frontend && docker compose run --rm -v "/Users/mac/项目/webmail-邮件系统/backend:/app" backend alembic -c alembic.ini upgrade head && python3 scripts/plan_status.py validate && git diff --check | 通过：联系人持久化模型、迁移与唯一约束落地，联系人和标签数据可持久化 | 2026-05-08T14:00:00+08:00 | 联系人持久化模型 |
| T36 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T35-T39 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_contacts_api.py backend/tests/test_send_mail_api.py -q && npm test --prefix frontend -- --run src/App.test.tsx && npm run build --prefix frontend && docker compose run --rm -v "/Users/mac/项目/webmail-邮件系统/backend:/app" backend alembic -c alembic.ini upgrade head && python3 scripts/plan_status.py validate && git diff --check | 通过：联系人弹层支持分页搜索、空状态和服务端分页接口联调 | 2026-05-08T14:00:00+08:00 | 联系人列表分页搜索 |
| T37 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T35-T39 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_contacts_api.py backend/tests/test_send_mail_api.py -q && npm test --prefix frontend -- --run src/App.test.tsx && npm run build --prefix frontend && docker compose run --rm -v "/Users/mac/项目/webmail-邮件系统/backend:/app" backend alembic -c alembic.ini upgrade head && python3 scripts/plan_status.py validate && git diff --check | 通过：联系人新增、编辑、删除与姓名/邮箱/手机/备注字段落地 | 2026-05-08T14:00:00+08:00 | 联系人 CRUD 与字段 |
| T38 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T35-T39 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_contacts_api.py backend/tests/test_send_mail_api.py -q && npm test --prefix frontend -- --run src/App.test.tsx && npm run build --prefix frontend && docker compose run --rm -v "/Users/mac/项目/webmail-邮件系统/backend:/app" backend alembic -c alembic.ini upgrade head && python3 scripts/plan_status.py validate && git diff --check | 通过：联系人分组与标签持久化，前端支持筛选与编辑 | 2026-05-08T14:00:00+08:00 | 联系人分组与标签 |
| T39 | 已完成 | Codex | feature/t01-project-plan-automation | WORKTREE-T35-T39 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_contacts_api.py backend/tests/test_send_mail_api.py -q && npm test --prefix frontend -- --run src/App.test.tsx && npm run build --prefix frontend && docker compose run --rm -v "/Users/mac/项目/webmail-邮件系统/backend:/app" backend alembic -c alembic.ini upgrade head && python3 scripts/plan_status.py validate && git diff --check | 通过：发送成功后自动收录/更新联系人，重复邮箱去重并累计使用次数 | 2026-05-08T14:00:00+08:00 | 自动收录联系人 |
| T40 | 已完成 | Codex | WORKTREE-T40-T42 | WORKTREE-T40-T42 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_contacts_persistence_api.py backend/tests/test_mailbox_real_imap_compat.py -q && npm test --prefix frontend -- --run src/App.test.tsx && npm run build --prefix frontend && python3 scripts/plan_status.py validate && git diff --check | 通过：联系人黑名单持久化、黑名单列表接口与刷新同步自动移入垃圾箱链路打通，黑名单命中后邮件从当前列表剔除 | 2026-05-08T15:05:00+08:00 | 联系人黑名单 |
| T41 | 已完成 | Codex | WORKTREE-T40-T42 | WORKTREE-T40-T42 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_contacts_persistence_api.py backend/tests/test_mailbox_real_imap_compat.py backend/tests/test_models.py -q && npm test --prefix frontend -- --run src/App.test.tsx && npm run build --prefix frontend && python3 scripts/plan_status.py validate && git diff --check | 通过：联系人白名单状态可持久化，白名单优先于黑名单，本地黑名单移动规则在白名单命中时跳过；并修复联系人测试夹具和模型映射污染问题 | 2026-05-08T15:05:00+08:00 | 联系人白名单 |
| T42 | 已完成 | Codex | WORKTREE-T40-T42 | WORKTREE-T40-T42 | PYTHONPATH=/Users/mac/项目/webmail-邮件系统/backend ./.venv/bin/pytest backend/tests/test_settings_api.py -q && npm test --prefix frontend -- --run src/App.test.tsx && npm run build --prefix frontend && python3 scripts/plan_status.py validate && git diff --check | 通过：系统设置提供修改密码入口，覆盖旧密码错误、新旧密码相同、IMAP 新密码校验失败和前端确认密码不一致不提交 | 2026-05-08T15:05:00+08:00 | 修改密码入口 |
| T43 | 未开始 |  |  |  |  |  |  | 语言与时区设置 |
| T44 | 未开始 |  |  |  |  |  |  | 回复引用方式设置 |
| T45 | 未开始 |  |  |  |  |  |  | 文件夹创建重命名删除 |
| T46 | 未开始 |  |  |  |  |  |  | 未读数角标一致性 |
...
| T26 | HTML 邮件安全渲染升级 | 前端使用 DOMPurify 白名单净化；当前阶段 HTML 邮件在正常阅读区直接富文本展示，暂不使用 sandbox iframe 隔离；纯文本邮件仍正常展示 | 后端 HTML 净化测试；前端 XSS 样例测试；Playwright 验证脚本不执行、样式表格图片正常 |
...
| T46 | 未读数角标一致性 | 文件夹未读数在列表刷新、已读未读切换、手动刷新后保持一致 | 后端未读计数测试；前端状态同步测试；Playwright 标记已读后角标减少 |
...
- 自动刷新：已按当前产品要求移除邮件工作台定时刷新；保留顶部手动刷新入口。
